...
public class FileUtils {
   ...
   public static String getDateFolder() {
      ...
      //File.separator: 각 OS 별로 경로구분자를 반환
   ...
   /*
    String uploadFolder : 서버측의 업로드될 폴더   C:\\dev\\upload\\product\\
...
//      File클래스  : 파일과 폴더관련 작업하는 기능
      ...
      try {
         //file : "C:/dev/upload/2023/11/02" + realUploadFileName : 업로드 할 파일명
         ...
         // 원본이미지를 파일크기의 해상도를 낮추어 작은형태의 이미지로 만드는 것
         if(checkImageType(saveFile)) {	//첨부된 파일이 이미지 일 경우라면
        	...
        	//	원본이미지파일의 내용을 스트림방식으로 읽어서 , 썸네일이미지파일에 쓰는작업
        	// 	 Thumbnailator.createThumbnail(원본이미지파일의 입력스트림, 썸네일파일 객체, 100 ,100)
         ...
         e.printStackTrace(); // 파일 업로드시 예외가 발생되면, 에러정보를 출력
      }



      return realUploadFileName; //상품테이블에 상품이미지명 으로 저장
   }
   
   //자바스크립트로 업로드 되는 파일의 확장자를 이용하여, 이미지 파일만 파일첨부 가능하도록 작업을 할수가 있다.
   //업로드 되는 파일구분 (이미지파일 또는 일반파일 구분)
   private static boolean checkImageType(File saveFile) {
   
   boolean isImageType = false; //변수의 값이 true 면 이미지 파일, false면 일반파일 구분작업
   
   
   
   try {
      //MIME정보: text/html, text / plain, image.jpeg......
      String contentType = Files.probeContentType(saveFile.toPath());
      
      //contentType 변수의 값이 image문자열로 시작되는지 여부를 true or false 값으로 리턴
